  ...
  return (
    <div className="w-full p-6 relative">
      <h1 className="font-bold text-xl">
        {username}{' '}
        {listingType == 'donate' ? 'is contributing' : 'is requesting for'}
        ...
      </h1>
      ...
      <>
        ...
        <div className="flex justify-between items-center mt-2">
          ...
          >
            ...
            {listingType === 'donate' ? 'claimed' : 'contributed'} of{' '}
...
>>>>>>> c2aa1e5e15939214569f1eef244e109fff41a278
        </div>
      </>
      ...
      <div className="text-xl font-bold flex">
        Comments
        <Badge className="ml-2 bg-gray-200 text-black">{comments.length}</Badge>
      </div>
      <CommentSection comments={comments} user={user} listingId={listingId} />
      <div className="sticky bottom-0 p-4 -mx-6 bg-white rounded-t-lg border-t-2 border-t-gray-200 drop-shadow-2xl pb-8">
        {user ? (
          <TransactionBtn user={user} listing={listing} listingOwner={owner} />
        ) : (
          <LoginButton
            text={listingType == 'receive' ? 'Contribute' : 'Request'}
          />
        )}
      </div>
    </div>
